...
def destination (Northing, Easting):
  ...
  start_point = {
    "longitude" : init_Easting,
    "latitude" : init_Northing
  }
  ...
  if (((end_point1["latitude"]-offset)<Northing<(end_point1["latitude"]+latitude_range)) and (end_point1["longitude"]-offset)<Easting<(end_point1["longitude"]+longitude_range)):
    output = 1
  else:
    output = 0
...
def callback_image(data):
  ...
  global line_parameter
  global state
  # U"canny_edges_min"]sed to convert between ROS and OpenCV images
  br = CvBridge()
  ...
  edges = cv2.Canny(grayImage, 45, 45)
